...
from PyQt5.QtWidgets import (QApplication, QWidget,QPushButton,QVBoxLayout,
                             QMessageBox, QPlainTextEdit) # 애플리케이션 핸들러와 빈 GUI 위젯, 버튼, layout, 메시지 박스 위젯, 텍스트 에디트 위젯 추가
...
class Calculator(QWidget): # QWidget 클래스를 상속받아서 클래스를 정의
    ...
    def initUI(self):
        ...
        self.te1.setReadOnly(True)    # 읽기만 가능하도록 수정
        ...
        self.setLayout(vbox)      # 빈 공간- 버튼- 빈 공간 순으로 수지 배치된 레이아웃 설정
    ...
    def activateMessage(self):    # 핸들러 함수 수정 : 메시지가 텍스트 에디터에 출력되도록
        # QMessageBox.information(self,"information","Button clicked!")
        self.te1.appendPlainText("Button clicked!")

if __name__ == '__main__':       # PyQt은 애플리케이션 당 1개의 QApplication이 필요함
